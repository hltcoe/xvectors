...
def train_with_freeze(model):
    ...
    if 0 and hasattr(model.embed.module, 'bnf'):
        # Turn off everything
        model.eval()
        for param in model.parameters():
            param.requires_grad = False
        # Turn on bnf
        model.embed.module.bnf.train()
        for param in model.embed.module.bnf.parameters():
            param.requires_grad = True
        logger.info(" Found embedding bnf layer, freezing rest...")
...
# Function for bottleneck feature mimic
def bnf(input_dim, layer_dim, output_dim):
    ...
    layers.extend(conv_block(2, layer_dim, layer_dim, 3, 2, 2))
    layers.extend(conv_block(3, layer_dim, layer_dim, 3, 3, 3))
    layers.extend(conv_block(4, layer_dim, output_dim, 3, 4, 4))

    return(nn.Sequential(OrderedDict(layers)))

class Xvector_embed(nn.Module):

    #def __init__(self, input_dim, layer_dim, embedding_dim, bn_momentum=0.1, relu_flag=False, conf_flag=False, bnf_dim=60):
    def __init__(self, input_dim, layer_dim, embedding_dim, bn_momentum=0.1, relu_flag=False, conf_flag=False, bnf_dim=None):

        super(Xvector_embed, self).__init__()

        # bottleneck_features
        if bnf_dim is not None:
            self.bnf = bnf(input_dim, layer_dim, bnf_dim)
            input_dim = bnf_dim
...
class Xvector9s(nn.Module):
    ...
    def __init__(self, input_dim, layer_dim, embedding_dim, num_classes, LL='linear', N0=9, fixed_N=True, length_norm=False, r=0.9, enroll_type='Bayes', loo_flag=True, bn_momentum=0.1, log_norm=True, conf_flag=False, resnet_flag=True, embed_relu_flag=False, bnf_dim=None):
